subroutine init_part
  ...
  !debugging
  integer :: Countnz,Count_7596=0,Count_5000=0,Count_11000=0,Count_40000=0,Count_1720=0
  ...
  allocate(accp  (npartmax,ndim)) !added by IT 2016
...
!  print *,'AAAAAAAAAAAAAAAAAAAAAAA npartmax =',npartmax,star,sink
!  allocate(tp(npartmax)); tp=0.0
  if(star.or.sink)then
!     print *,'BBBBBBBBBBBBBBBBBBBBBBB npartmax =',npartmax
!     stop
  ...
  if(nrestart>0)then
     ...
     open(10,file='n_particles.txt',status='old')
     read(10,*) i_end_M31_all, i_end_MW_all, gal_axis1(1), gal_axis1(2), gal_axis1(3), gal_axis2(1), gal_axis2(2), gal_axis2(3)
     close(10)
     ...
     ! Read accelerations (IT2018)
     do idim=1,ndim
        read(ilun)xdp
        accp(1:npart2,idim)=xdp
     enddo
     ! Read mass
     read(ilun)xdp
     mp(1:npart2)=xdp
     ...
     case ('ascii')
        ...
        scale_m=1.d9*scale_d*scale_l**3!IT2015-11-10: added factor 1.d9
	...
	if(myid==1)then
           open(10,file=filename,form='formatted',action='read')
	   write(6,*) "hello, filename is ",filename  		!Added by STN
           indglob=0
        end if
        eof=.false.
        ...
        do while (.not.eof)
           ...
           if(myid==1)then
              ...
              do i=1,nvector
                 ...
                 vv1=vv1*1D5/scale_v! * sqrt1d9!added conversion by IT 2015-11-10
                 vv2=vv2*1D5/scale_v! * sqrt1d9
                 vv3=vv3*1D5/scale_v! * sqrt1d9
                 ...
                 vv(i,:) = (matmul(Rot_gal, (/ vv1, vv2, vv3 /)) + Vgal1)! * scale_kpc_to_km!added conversion
                 mm(i  ) = mm1
                 ii(i  ) = indglob
              end do
100           continue
...
103           continue
        ...
        if(myid==1)close(10)
        i_end_MW=ipart
        !print *,'>>>> i_end_MW = ',i_end_MW
        !print *,'>>>> Min of levelp = ',minval(levelp)
        !print *,'>>>> Max of levelp = ',maxval(levelp)
        !Countnz = 0
        !do i = 1,npartmax
        ...
        !   Countnz = Countnz + 1
        !end do
        !print *,'>>>> Non-zero elements in levelp = ', Countnz
        !print *,'>>>> Min of idp = ',minval(idp)
        !print *,'>>>> Max of idp = ',maxval(idp)
        !Countnz = 0
        !do i = 1,npartmax
        !   if (levelp(i) .eq. 0) then
        !      cycle
        !   end if
        !   Countnz = Countnz + 1
        !   if (idp(i) .le. 5000) then
        !      Count_5000 = Count_5000 + 1
        !   end if
        !   if (idp(i) .le. 11000) then
        !      Count_11000 = Count_11000 + 1
        !   end if
        !   if (idp(i) .le. 40000) then
        !      Count_40000 = Count_40000 + 1
        !   end if
        !   if (idp(i) .le. 3796) then
        !      Count_7596 = Count_7596 + 1
        !   end if
        !   if (idp(i) .le. 8720) then
        !      Count_1720 = Count_1720 + 1
        !   endif
        !end do
        !print *,'>>>> Non-zero elements in idp = ', Countnz
        !print *,'>>>> indglob  = ',indglob
        !print *,'>>>> idp5000  = ', Count_5000
        !print *,'>>>> idp11000 = ', Count_11000
        !print *,'>>>> idp40000 = ', Count_40000
        !print *,'>>>> idp7596  = ', Count_7596
        !print *,'>>>> idp1720  = ', Count_1720
        end if
        !######################################################################
        ...
        call MPI_ALLREDUCE(npart_cpu,npart_all,ncpu,MPI_INTEGER,MPI_SUM,MPI_COMM_WORLD,info)
        npart_cpu(1)=npart_all(1)
        call MPI_ALLREDUCE(i_end_MW,i_end_MW_all,1,MPI_INTEGER,MPI_SUM,MPI_COMM_WORLD,info)
        call MPI_ALLREDUCE(i_end_M31,i_end_M31_all,1,MPI_INTEGER,MPI_SUM,MPI_COMM_WORLD,info)
#endif
        do icpu=2,ncpu
           npart_cpu(icpu)=npart_cpu(icpu-1)+npart_all(icpu)
        end do
        ...
        if (myid==1) then
           ...
           open(10,file='n_particles.txt')
           write(10,*) i_end_M31_all, i_end_MW_all, gal_axis1(1), gal_axis1(2), gal_axis1(3), gal_axis2(1), gal_axis2(2), gal_axis2(3)
           close(10)
        endif
     case ('gadget')
        call load_gadget
