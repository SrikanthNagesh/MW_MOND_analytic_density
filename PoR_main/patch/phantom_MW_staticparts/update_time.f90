...
#endif
           !~~~~~~~~~ begin ~~~~~~~~~
           ! Not related to MOND: Changed time output 
           ! from [user time units] to [Myr]
           !
           if(pic)then
              write(*,888)nstep,t*user_time_to_Myr,dt*user_time_to_Myr,aexp,&
...
subroutine getmem(outmem)
  ...
  file='/proc/self/stat'
  inquire(FILE=file, EXIST=proc_exists)
  close(1)
  if (proc_exists) then
      open(unit=1,file=file,form='formatted')
      read(1,'(A300)',IOSTAT=read_status)dir
  else if (myid==1) then
      write (*,*) 'Warning: /proc does not exist'
  endif
  if (proc_exists .and. read_status < 0) then
     outmem=dble(0.)
