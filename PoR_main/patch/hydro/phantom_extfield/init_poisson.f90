subroutine init_poisson
  ...
  if(nrestart>0)then
     ...
     !print '(3a)','fileloc = "',trim(fileloc),'"'
     open(unit=ilun,file=TRIM(fileloc),form='unformatted')
     read(ilun)ncpu2
     read(ilun)ndim2
     read(ilun)nlevelmax2
     read(ilun)nboundary2
     !print *,'ncpu2,ndim2,nlevelmax2,nboundary2 =',ncpu2,ndim2,nlevelmax2,nboundary2
           ...
           if(ncache>0)then
              ...
              if (mond) then
                 ...
                 do ind=1,twotondim
                    ...
                    read(ilun)xx
                    do i=1,ncache
                       !if (xx(i)/=0.d0) print *,'Newton: i,xx(i) =',i,xx(i)
                    ...
                    do ivar=1,ndim
                       read(ilun)xx
                       do i=1,ncache
                          !if (xx(i)/=0.d0) print *,'MOND: i,xx(i) =',i,xx(i)
