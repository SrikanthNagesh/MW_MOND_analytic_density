...
subroutine rho_fine(ilevel,icount)
  ...
  !------------------------------------------------------------------
  integer::iskip,icpu,ind,i,info,nx_loc,ibound,idim,ncell
...
subroutine multipole_fine(ilevel)
  ...
  !-------------------------------------------------------------------
  integer ::ind,i,icpu,ncache,igrid,ngrid,iskip,info,ibound,nx_loc,ncell
...
subroutine cic_from_multipole(ilevel)
  ...
  ! solver, the restriction is necessary in any case.
  !-------------------------------------------------------------------
  integer ::ind,i,j,icpu,ncache,ngrid,iskip,info,ibound,nx_loc,ncell
  ...
  if(hydro .or. gravity_type < 0)then
...
subroutine cic_cell(ind_grid,ngrid,ilevel)
  ...
  !
  !
  integer::i,j,idim,ind_cell_son,iskip_son,np,ind_son,nx_loc,ind,ncell
  ...
  logical::error
    ...
    write(*,*)"We entered cic_cell subrouting"
  ...
  do ind_son=1,twotondim
     ...
     do idim=1,ndim
        write(*,*)"computing pseudo particle COM"
        do j=1,np
	...
	write(*,*)"np and ngrid", np, ngrid
     do ind=1,twotondim
	write(*,*)"This is where things get weird"
       do j=1,np
	write(*,*)"j, ind, kg(j,ind)", j, ind, kg(j,ind), kg(2,1)
	write(*,*)nbors_father_cells(j,kg(j,ind))
	write(*,*)son(nbors_father_cells(j,kg(j,ind)))
