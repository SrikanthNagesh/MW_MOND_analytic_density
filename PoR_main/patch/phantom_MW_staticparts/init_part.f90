subroutine init_part
  ...
  if(nrestart>0)then
     ...
     case ('ascii')
        ...
        scale_m=scale_d*scale_l**3 ! Original scale_m for staticparts patch
	!scale_m=1.d9*scale_d*scale_l**3!IT2015-11-10: added factor 1.d9 !Pasted here by Srikanth Nagesh
        !~~~~~~~~~~ end ~~~~~~~~~~
	...
	filename=initfile(levelmin) !Fixed by EA
        if(myid==1)then
        ...
        do while (.not.eof)
           ...
           if(myid==1)then
              ...
              do i=1,nvector
                 ...
                 !~~~~~~~~~ begin ~~~~~~~~~
                 ! Changed scaling from user units to astrophysical units (km/s)
                 !
                 !vv(i,1)=vv1 * scale_t / scale_kpc_to_km  ! @FL  Added conversion 
                 !vv(i,2)=vv2 * scale_t / scale_kpc_to_km  ! @FL  from km/s to [kpc]/[user time unit]
                 !vv(i,3)=vv3 * scale_t / scale_kpc_to_km  ! @FL  (user length unit = kpc)
                 vv(i,1)=vv1 * 1D5/scale_v ! @FL  Added conversion 
                 vv(i,2)=vv2 * 1D5/scale_v ! @FL  from km/s to [kpc]/[user time unit]
                 vv(i,3)=vv3 * 1D5/scale_v ! @FL  (user length unit = kpc)
		 !mm1=mm1*1D9*1.9891D33/scale_m !Added by Srikanth Nagesh
                 !~~~~~~~~~~ end ~~~~~~~~~~
                 mm(i  )=mm1*1.9891D33/scale_m !Original line
		 mm(i  )=mm1
                 ii(i  )=indglob
              end do
100           continue
