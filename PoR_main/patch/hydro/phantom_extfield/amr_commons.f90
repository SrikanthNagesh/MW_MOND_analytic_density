module amr_commons
  ...
  !~~~~~~~~~ begin ~~~~~~~~~
  ! Make this a pointer
  real(dp),pointer::rho_tot                             ! Mean density in the box
  !~~~~~~~~~~ end ~~~~~~~~~~
  ...
  !~~~~~~~~~ begin ~~~~~~~~~ [units]
  ! These are optional, not MOND-related changes
  ! to scale the input file, and the time output
  real(dp)::scale_Myr_to_seconds=3.15576D+13 ! [3.15576e+13 s / Myr]
  real(dp)::scale_kpc_to_km=3.08567758D+16   ! [3.08568e+16 km / kpc]
! added by IT 2015-11-10
  real(dp)::sqrt1d9=3.16227766016838d4
! /
  real(dp)::user_time_to_Myr
  real(dp)::kpc_per_UT_to_km_per_s
  !~~~~~~~~~~ end ~~~~~~~~~~

  !Auxiliary variables (IT 2019-02-19)
  real(dp), dimension(3) :: axis1_dummy,axis2_dummy
  integer :: safety_secs=1
