...
subroutine read_merger_params! {{{
  ...
  real(dp),dimension(3)::vdum0=(/0.d0, 0.d0, 1.d0/),vdum1
  ...
  namelist/merger_params/ IG_density_factor &
       ...
       & ,typ_radius_inner1,mass_frac_inner1 &
       & ,typ_radius_inner2,mass_frac_inner2 &
...
! added by IT 2015-11-24--2017-06-02--20180702
       & ,scale_vcirc,scale_vigal,scale_a2,T2_ISM &
       & ,devflat_dens,devflat_vel,devscal_dens,devscal_vel &
       & ,scale_objsize,flg_qqm3d,flg_dmin,flg_axsph
...
!  use qqm3d_module, only : qqm_nx,qqm_ny,qqm_nz,afield
  ...
  if (T2_ISM<=0.d0) then
     a2=1d4 / scale_T2 * scale_a2! sound speed squared times user-defined scaling factor (default=1.0)
  else
  ...
  select case (rad_profile) !IT added this block to replace former rho_0_1 evaluation method.
      ...
      case ('Toomre')
         rho_0_1 = Mgas_disk1 / (4.0D0 * pi * typ_radius1**2 * typ_height1 * rho_0_1)
      case ('double_exp')
         uu = mass_frac_inner1*typ_radius_inner1**2 + mass_frac_outer1*typ_radius1**2
  ...
  select case (rad_profile) !IB added this block to replace former rho_0_2 evaluation method.
      ...
      case ('Toomre')
         rho_0_2 = Mgas_disk2 / (4.0D0 * pi * typ_radius2**2 * typ_height2 * rho_0_2)
      case ('double_exp')
         uu = mass_frac_inner2*typ_radius_inner2**2 + mass_frac_outer2*typ_radius2**2
  ...
  select case (rad_profile)
      ...
      case ('double_exp') !double_exp case added by IB.
          u_1 = cut_radius1 / typ_radius_inner1
          u_2 = cut_radius1 / typ_radius1
          dmin1 = rho_0_1*(mass_frac_inner1*exp(-u_1) + mass_frac_outer1*exp(-u_2))
          u_1 = cut_radius2 / typ_radius_inner2
          u_2 = cut_radius2 / typ_radius2
          dmin2 = rho_0_2*(mass_frac_inner2*exp(-u_1) + mass_frac_outer2*exp(-u_2))
      case ('Toomre')
  ...
  select case (z_profile)
      ...
      case ('sech_sq') !IB added the sech_sq case.
          dmin1 = dmin1 / (cosh(cut_height1 / typ_height1))**2
          dmin2 = dmin2 / (cosh(cut_height2 / typ_height2))**2
  end select
  ...
  !print '(a,2(2x,e12.4))','-------- CONDINIT: IG_density_factor,dmin =',IG_density_factor,dmin
        ...
        select case (rad_profile)
            ...
            case ('double_exp') !IB added the double_exp case.
                if (ind_gal .eq. 1) then
                    ...
                    u_2 = r/typ_radius1
                    q(i, 1) = rho_0*(mass_frac_inner1*exp(-u_1) + mass_frac_outer1*exp(-u_2))
                else
                    u_1 = r/typ_radius_inner2
                    u_2 = r/typ_radius2
                    q(i, 1) = rho_0*(mass_frac_inner2*exp(-u_1) + mass_frac_outer2*exp(-u_2))
                end if
            case ('Toomre')
...
!                Vrot = sqrt(max(Vcirc**2 - 3.0D0*a2 - r/rgal * aa,0.0D0))! IT 2017-01-11
            case ('double_exp') !IB added the double_exp case and used the above formula.
                if (ind_gal .eq. 1) then
                    ...
                    Vrot = sqrt(max(Vcirc**2 - 3.0D0*a2 - uu * a2,0.0D0))
                else
                    u_1 = r/typ_radius_inner2
                    u_2 = r/typ_radius2
...
!                Vrot = sqrt(max(Vcirc**2 - 3.0D0*a2 - r**2/(r**2+rgal**2)*aa,0.0D0))! IT 2017-01-11
        end select
        Vrot = weight * Vrot * scale_vcirc(ind_gal)!scale_vcirc factor added by IT 2015-11-24
