...
module mond_parameters
  ...
  real(dp) :: a0_ms2 = 1.2e-10    ! a0 in m/s^2
...
!  Notice again: it is IMPORTANT that nu(x)->0 if x>>1
   if(trim(nu_function)=='simple') then
	   nu = -0.5 + sqrt(0.25 + 1.0/x)              ! Simple nu function
   else if((trim(nu_function)=='standard') .OR. (trim(nu_function)== 'std')) then	
	  nu = sqrt( 0.5 + sqrt( 0.25 + 1.0/x**2 ) ) - 1.0 ! Standard nu function
	else if (trim(nu_function)=='exp') then
		nu = (1.0-exp(-x*x))**(-0.25) + (0.75)*exp(-x*x) - 1.0 ! exponentiel nu function
	else if (trim(nu_function)=='newton') then
		nu = 0.0 ! newton
	endif
