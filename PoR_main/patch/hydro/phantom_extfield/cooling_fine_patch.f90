...
subroutine coolfine1(ind_grid,ngrid,ilevel)
  ...
  real(kind=8),dimension(1:nvector),save::nH,T2,delta_T2,ekk,T2min,Zsolar, boost
  ...
  do ind=1,twotondim
     ...
     do i=1,nleaf
        ...
        if(nH(i) .LT. -1.0D-3) then ! effectively switched off dodgy if-clause (IB, IT 2018)
            T2min(i) = 4.0D6*(nH(i) / 1.0D-3)**(gamma - 1.0D0)
        else
            select case (eos_type)
                case ('isothermal') ! Isothermal EOS
                    T2min(i) = T2_star
                case ('pseudo_cooling') ! Pseudo-cooling EOS
                    !if(nH(i) .LT. 10.0D0**(-0.5D0)) then ! Isothermal T2_star EOS
                    if(nH(i) .LT. 3.162277660168379332d-1) then ! Isothermal T2_star EOS
                        T2min(i) = T2_star
                    else ! Cooling EOS
                        !T2min(i) = T2_star * (nH(i)/10.0D0**(-0.5D0))**(-1.0D0/2.0D0)
                        T2min(i) = T2_star * (3.162277660168379332d0*nH(i))**(-1.0D0/2.0D0)
                    end if
