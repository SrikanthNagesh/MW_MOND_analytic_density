subroutine adaptive_loop
  ...
  !~~~~~~~~~ begin ~~~~~~~~~
  if(mond)   call init_mond          ! Initialize variables
  !~~~~~~~~~~ end ~~~~~~~~~~

  if (nrestart>0) then
     open(unit_output_centres,file='centre_info',position='append')
  else
     open(unit_output_centres,file='centre_info')
  endif
     ...
     if(levelmin.lt.nlevelmax .and..not.static)then
        ...
        do ilevel=1,levelmin
           ...
           if(poisson)then
              ...
              if (mond) then
                 ! Newton
                 call make_virtual_fine_dp(phi_newton(1),ilevel)
                 do idim=1,ndim
                    call make_virtual_fine_dp(f_newton(1,idim),ilevel)
                 end do
                 ! MOND
                 call make_virtual_fine_dp(phi_mond(1),ilevel)
                 do idim=1,ndim
                    call make_virtual_fine_dp(f_mond(1,idim),ilevel)
                 end do
              else
